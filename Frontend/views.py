...
# def ExamBox(request):
#     try:
#         exams_item  = request.user.groups.all()[0].exam_set.order_by('-exam_permission','-exam_available_time_end',)[0]
#         exams_avg   = request.user.examaverage
#     except:
...
# def AssignmentBox(request):
#     try:
#         assignment_listed   = request.user.groups.all()[0].assignment_set.order_by('-assignment_permission','-assignment_available_time_end',)[:3]
#         assignment_avg      = request.user.assignmentaverage
#     except:
#         return Response(status=status.HTTP_404_NOT_FOUND)
#     else:
#         listed_assignment = list(assignment_listed)
#         assignment_dict = []
#         for i in listed_assignment:
#             assignment_dict.append(A_serializer.AssignmentSerializer(i).data)
#         return Response((assignment_dict,A_serializer.AssignmentAverageSerializer(assignment_avg).data),status=status.HTTP_200_OK)
